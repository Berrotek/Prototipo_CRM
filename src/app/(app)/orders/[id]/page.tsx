...
export default async function OrderDetailPage({ params }: { params: { id: string } }) {
  ...
  const { data: order } = await supabase
    .from('sales_orders')
    .select('id, order_number, customer_id, fulfillment_location_id, status, currency, subtotal, tax_total, discount_total, shipping_total, total, required_by, notes, created_at')
    .eq('id', params.id)
  ...
  const { data: items } = await supabase
    ...
    .select('id, line_number, description, quantity, unit_price, tax_rate, discount_total, line_total, product_id')
    ...
    <div className="space-y-8">
      <section className="flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-2xl font-semibold text-white">{order.order_number}</h1>
          <p className="mt-2 text-sm text-neutral-400">
            Order detail shell for status changes, fulfillment progress, and later invoice linkage.
          </p>
        </div>

        <Link href="/orders" className="rounded-md border border-neutral-800 px-4 py-2 text-sm text-neutral-200">
          Back to orders
        </Link>
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-neutral-800 bg-neutral-950 p-5">
            ...
            <p className="mt-2 text-sm text-neutral-400">{order.notes ?? 'No notes provided.'}</p>
          </div>

          <div className="rounded-2xl border border-neutral-800 bg-neutral-950 p-5">
            <h2 className="text-sm font-medium text-white">Next actions</h2>
            <p className="mt-2 text-sm text-neutral-400">
              The next real workflow will be confirmation, allocation, and fulfillment progress.
            </p>
          </div>
        </div>
      </section>
